...
Sometimes two variables can have the same value but be **different objects** in memory. \`==\` checks value; \`is\` checks identity. The most common use is checking for [[NoneType|None]]: \`if x is None\`.
...
- **Value comparison:** [[Comparison]] (\`==\` checks value, \`is\` checks identity)
- **Used with:** [[NoneType]] (\`x is None\` is the standard pattern)
- **Logical combinations:** [[Logical]] (\`and\`, \`or\`, \`not\`)
- **Results are:** [[bool]]
- **Parent:** [[00_Index]]
`;export{e as default};