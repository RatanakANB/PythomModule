...
2. \`.intersection()\` → [[Set_Methods|Set method]] that finds items in **both** sets
...
- **Parent:** [[Variables]]
- **Methods:** [[Set_Methods]] (17 methods — add, remove, union, intersection, etc.)
- **Similar types:** [[list]] (ordered, allows duplicates), [[tuple]] (ordered, immutable), [[dict]] (also uses \`{}\` but with key-value pairs)
- **Operators:** [[Membership]] (\`in\` is extremely fast for sets), [[Comparison]] (subset/superset checks)
- **Key concept:** [[Mutability_vs_Immutability]] — sets are **mutable** (can add/remove items, but items themselves must be immutable)
`;export{e as default};