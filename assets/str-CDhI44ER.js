...
**Step-by-step:**
1. \`first_name = "Sophia"\` → Stores the text \`Sophia\` as a [[Variables|variable]]
2. \`full_name = first_name + " " + last_name\` → The \`+\` [[Arithmetic|operator]] joins strings together
3. \`.upper()\` → A [[String_Methods]] that returns ALL CAPS version
4. \`.replace("-", "")\` → Replaces all hyphens with nothing (removes them)
5. The original strings are **unchanged** — strings are **immutable** (see [[Mutability_vs_Immutability]])
...
- **Parent:** [[Variables]]
- **Methods:** [[String_Methods]] (47 methods for working with text)
- **Related types:** [[int]] (convert with \`int()\`), [[float]] (convert with \`float()\`), [[list]] (convert with \`list()\`)
- **Operators:** [[Arithmetic]] (\`+\` joins strings), [[Comparison]] (\`==\` compares strings), [[Membership]] (\`in\` checks substring)
- **Key concept:** [[Mutability_vs_Immutability]] — strings are **immutable**
`;export{e as default};