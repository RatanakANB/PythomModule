...
**Step-by-step:**
1. \`first_name = "Sophia"\` → Stores the text \`Sophia\` as a [[1.01_Variables|variable]]
2. \`full_name = first_name + " " + last_name\` → The \`+\` [[3.01_Arithmetic|operator]] joins strings together
3. \`.upper()\` → A [[2.05_String_Methods]] that returns ALL CAPS version
4. \`.replace("-", "")\` → Replaces all hyphens with nothing (removes them)
5. The original strings are **unchanged** — strings are **immutable** (see [[2.02_Mutability_vs_Immutability]])
...
- **Methods:** [[2.05_String_Methods]] (47 methods for working with text)
- **Related types:** [[1.03_Interger]] (convert with \`int()\`), [[1.04_Float]] (convert with \`float()\`), [[1.07_list]] (convert with \`list()\`)
- **Indexing & Slicing:** [[4.01_Indexing]] (grab one letter), [[4.02_Slicing]] (grab a section)
- **Operators:** [[3.01_Arithmetic]] (\`+\` joins strings), [[3.04_Comparison]] (\`==\` compares strings), [[3.05_Membership]] (\`in\` checks substring)
- **Key concept:** [[2.02_Mutability_vs_Immutability]] — strings are **immutable**
`;export{e as default};