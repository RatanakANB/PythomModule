...
Sometimes two variables can have the same value but be **different objects** in memory. \`==\` checks value; \`is\` checks identity. The most common use is checking for [[1.06_NoneType|None]]: \`if x is None\`.
...
- **Value comparison:** [[3.04_Comparison]] (\`==\` checks value, \`is\` checks identity)
- **Used with:** [[1.06_NoneType]] (\`x is None\` is the standard pattern)
- **Logical combinations:** [[3.03_Logical]] (\`and\`, \`or\`, \`not\`)
