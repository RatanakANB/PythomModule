...
Sometimes a [[list]] isn't enough — you don't just want items in order, you want to **label** each item. Dictionaries let you store data with meaningful names instead of just positions.
...
3. \`profile["followers"] = 843\` → Change a value — dicts are **mutable** (see [[Mutability_vs_Immutability]])
4. \`.keys()\` and \`.values()\` → [[Dict_Methods]] to get all keys or all values
...
- **Parent:** [[Variables]]
- **Methods:** [[Dict_Methods]] (11 methods — get, keys, values, items, etc.)
- **Similar types:** [[list]] (ordered by index), [[set]] (also uses \`{}\` but no values)
- **Related types:** [[str]] (keys are often strings), [[int]] (values can be any type)
- **Operators:** [[Membership]] (\`in\` checks if a KEY exists), [[Comparison]] (compare dicts)
- **Key concept:** [[Mutability_vs_Immutability]] — dicts are **mutable**
`;export{e as default};