...
3. \`.append("Music")\` → [[List_Methods|List method]] that adds to the end
4. \`.remove("Art")\` → Finds and removes \`"Art"\`
5. \`.sort()\` → Sorts the list in place — changes the original! (see [[Mutability_vs_Immutability]])
...
- **Parent:** [[Variables]]
- **Methods:** [[List_Methods]] (11 methods — append, sort, remove, etc.)
- **Similar types:** [[tuple]] (like a list but can't change), [[set]] (like a list but no duplicates)
- **Related types:** [[str]] (also ordered and indexable), [[dict]] (stores key-value pairs instead)
- **Operators:** [[Arithmetic]] (\`+\` joins lists), [[Membership]] (\`in\` checks if item exists), [[Comparison]] (compare lists)
- **Key concept:** [[Mutability_vs_Immutability]] — lists are **mutable** (can change)
`;export{e as default};